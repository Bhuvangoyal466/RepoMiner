...
def generate_onboarding_markdown(
    repo_root: Path, stats: Dict[str, Any] | None = None
) -> str:
    """Generate a pragmatic onboarding doc."""

    stats = stats or {}
    has_py = (repo_root / "requirements.txt").is_file() or any(
        p.suffix == ".py" for p in repo_root.rglob("*.py")
    )
...
def generate_summary_markdown(
    ...
    **_legacy: Any,
    ...
    top_languages = (
        sorted(lang_counts.items(), key=lambda kv: -kv[1])[:5]
    ...
    # 5. Module structure
    lines += ["## 4. Module Structure", "", "Top-level directories by file count:", ""]
    if modules:
        lines += ["| Module | Files |", "|---|---:|"]
